...
import com.attech.thenutapi.common.exception.ThenutApiBusinessException;
...
public class RestExceptionHandler extends BaseController {
    ...
    @ExceptionHandler(ThenutApiBusinessException.class)
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public Response<ErrorResponse> handleThenutApiBusinessException(ThenutApiBusinessException ex, Locale locale) {
        return createErrorResponseFromMessageSource(ex.getKey(), locale, ex.getArgs());
    }

    @ExceptionHandler(ThenutApiDataNotFoundException.class)
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public Response<ErrorResponse> handleThenutApiDataNotFoundException(ThenutApiDataNotFoundException ex, Locale locale) {
        return createErrorResponseFromMessageSource(ex.getKey(), locale, ex.getArgs());
    }
    ...
    public Response<ErrorResponse> handleMethodArgumentTypeMismatchException(
        ...
        return createErrorResponseFromMessageSource(
                "common.client.typeMismatch",
                locale, methodArgumentTypeMismatchException.getName()
        );
    }
    ...
    public Response<ErrorResponse> handleMethodNotSupportedException(
        ...
        return createErrorResponseFromMessageSource(
                "common.client.methodNotSupported",
                locale, methodNotSupportedException.getMethod()
        );
    }

    private Response<ErrorResponse> createFieldErrorResponse(BindingResult bindingResult, Locale locale) {
        var requiredFieldErrorMessages = retrieveLocalizationMessage("common.client.requiredField", locale);
        var code = requiredFieldErrorMessages.get(0);
        var errorMessage = bindingResult
    ...
    private Response<ErrorResponse> createErrorResponseFromMessageSource(String key, Locale locale, String... args) {
        var messageList = retrieveLocalizationMessage(key, locale, args);
    ...
    private List<String> retrieveLocalizationMessage(String key, Locale locale, String... args) {
        var message = messageSource.getMessage(key, args, locale);
